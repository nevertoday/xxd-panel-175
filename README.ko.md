...
# XXD Panel 175｜호박빛 역광의 상아색 종이 조각
...
종이 조각은 20–25%, 여백은 75–80%입니다. 따뜻한 상아색·호박빛 역광·따뜻한 갈색 그림자를 쓰며 빨강이나 짙은 먹색 강조는 1% 미만입니다.
...
- `design-only`: 전체 캔버스에 Panel 175의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-175`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-175 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-175-prompt.en.md)와 [중국어](references/xxd-panel-175-prompt.zh-CN.md)를 확인하세요.
